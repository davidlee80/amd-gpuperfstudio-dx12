...
gtString::gtString(const gtString& otherString)
    : _impl(otherString._impl), _stringAsASCIICharArray(NULL)
{
}
...
gtString::gtString(const wchar_t* pOtherString)
    : _impl((pOtherString != NULL) ? pOtherString : L""), _stringAsASCIICharArray(NULL)
{
}

gtString::gtString(const wchar_t* pOtherString, int len)
    : _impl(pOtherString, len), _stringAsASCIICharArray(NULL)
{
}
...
{
    otherString._stringAsASCIICharArray = NULL;
}
...
gtString& gtString::operator=(gtString&& otherString)
{
    if (this != &otherString)
    {
        ...
        if (NULL != _stringAsASCIICharArray)
        {
            delete[] _stringAsASCIICharArray;
        }
...
// Description: Return this string as a null terminated wchar array.
// Author:      AMD Developer Tools Team
// Date:        16/11/2003
// ---------------------------------------------------------------------------
...
gtString& gtString::appendFormattedString(const wchar_t* pFormatString, ...)
{
...
#else
    {
        ...
        int formattedStringSize = vswprintf(NULL, 0, pFormatString, argptr2);
...
bool gtString::isEqual(const char* pOtherString)
{
    ...
    else // pOtherString == NULL
    {
        // The NULL string is equal only to the empty string:
        retVal = isEmpty();
    }
...
// Name:        gtString::trim
// Description:
//   Removes lerading and trailing whitespace characters from the string.
//   Example:
//   gtString foo = "  abc  ";
//   foo.trim();
//   Will yield "abc" string
...
int gtUtf8StringToWideString(const std::string& org, std::wstring& dst)
{
    ...
    int rc = ::MultiByteToWideChar(CP_UTF8,          // Code Page = UTF8
                                   0,                // Use the default flags
                                   org.data(),       // Source wide string
                                   orgLength,        // WideCharToMultiByte will deduce the string length based on null termination
...
#endif // #if GR_BUILD_TARGET == GR_WINDOWS_OS


// -------------------------------------------------------------------------------------------
// Name:        operator<<
// Description: Friend function of gtString, allows gtString to accept null-terminated
...
// -------------------------------------------------------------------------------------------
// Name:        operator<<
// Description: Friend function of gtString, allows gtString to accept null-terminated wide
//              character strings as an input.
// Arguments:   gtString& target      - the target stream
