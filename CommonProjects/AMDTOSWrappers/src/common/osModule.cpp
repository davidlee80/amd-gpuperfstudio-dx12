...
#else // ! GR_LINUX_VARIANT == GR_MAC_OS_X_LINUX_VARIANT
    {
        ...
        gtString GLDriverPathEnvVariableValue;
        bool hasGLDriverPathEnvVariable = osGetCurrentProcessEnvVariableValue(GLDriverPathEnvVariable, GLDriverPathEnvVariableValue);

        // If we have such an environment variable - use it's content as the system's OpenGL module's directory:
        if (hasGLDriverPathEnvVariable)
        {
            // On 64-bit linux machines, this is set to /usr/lib/dri:/usr/lib64/dri. We must search both paths:
            gtStringTokenizer driverPathTokenizer(GLDriverPathEnvVariableValue, osFilePath::osEnvironmentVariablePathsSeparator);

            while (driverPathTokenizer.getNextToken(systemOGLModulePathStr))
            {
                systemOGLModulePathStr.removeTrailing(osFilePath::osPathSeparator).append(osFilePath::osPathSeparator).append('.').append('.');
                ...
                osFilePath systemOGLModulePathAsFilePath(systemOGLModulePathStr);
        ...
        // DEV463567: On Linux, add a few more hard-coded fallback locations, in case the LIBGL_DRIVERS_PATH is not set correctly.
        // This is ugly and I don't like it =(
#if ((GR_BUILD_TARGET == GR_LINUX_OS) && (GR_LINUX_VARIANT == GR_GENERIC_LINUX_VARIANT))
        {
            systemOGLModulePathStr = L"/usr/lib/fglrx/" OS_OPENGL_MODULE_NAME;
            systemOGLModulePaths.push_back(systemOGLModulePathStr);
            systemOGLModulePathStr = L"/usr/lib/fglrx/dri/" OS_OPENGL_MODULE_NAME;
            systemOGLModulePaths.push_back(systemOGLModulePathStr);
            systemOGLModulePathStr = L"/usr/lib/x86_64-linux-gnu/" OS_OPENGL_MODULE_NAME;
            systemOGLModulePaths.push_back(systemOGLModulePathStr);
            systemOGLModulePathStr = L"/usr/lib/x86_64-linux-gnu/mesa/" OS_OPENGL_MODULE_NAME;
            systemOGLModulePaths.push_back(systemOGLModulePathStr);
        }
#endif
    }
