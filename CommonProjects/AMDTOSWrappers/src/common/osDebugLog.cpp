...
// Date:        30/8/2005
// ---------------------------------------------------------------------------
bool osDebugLog::initialize(const gtString& logFileName, const wchar_t* pszProductDescription, const wchar_t* pszOSDescription)
{
    bool retVal = false;
    ...
    calculateLogFilePath(logFileName, logFilePath);

    // Initialize the log file:
    bool rc1 = initialize(logFilePath);
                             ...
                             const wchar_t* message, osDebugLogSeverity severity)
{
    ...
    if (_isInitialized)
    {
        // If the message severity is "higher" than the current debug log logged severity:
        if (severity <= _loggedSeverity)
        {
...
#if GR_BUILD_TARGET == GR_WINDOWS_OS
#if GR_BUILD_CONFIGURATION == GR_DEBUG_BUILD
...
void osDebugLog::calculateLogFilePath(const gtString& logFileName, osFilePath& logFilePath)
{
    ...
    osFilePath debugLogFilePath(osFilePath::OS_TEMP_DIRECTORY);
...
#if GR_BUILD_CONFIGURATION == GR_RELEASE_BUILD
        sessionStartMessage += OS_STR_ReleaseVersion;

#elif GR_BUILD_CONFIGURATION == GR_DEBUG_BUILD
