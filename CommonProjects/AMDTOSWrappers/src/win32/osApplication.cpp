...
void osGetApplicationVersion(osProductVersion& applicationVersion)
{
    ...
    DWORD rc = GetModuleFileName(NULL, fileName, 4096);
    ...
    if (rc != 0)
    {
        ...
        // Read the data directly from the executable file details unless we're running inside Visual Studio as
        // a VS extension, in which case we'll use the version macros after this 'if' block to avoid getting
        // Visual Studio's version number
