...
/*
bool LayerManager::PushLayer( string name )
{

   std::vector<ILayer *>::iterator it;
   for ( it=m_AvailableLayers.begin() ; it < m_AvailableLayers.end(); it++ )
   {
      ILayer *pL = *it;

      if ( name.compare( pL->GetName() ) == 0 )
      {
         m_AvailableLayers.erase( it );

         m_EnabledLayers.push_back( pL );
...
bool LayerManager::PopLayer()
{
   if ( m_EnabledLayers.empty() == false )
   {
      ILayer *pL = m_EnabledLayers.back();
      m_EnabledLayers.pop_back();

      m_AvailableLayers.push_back( pL );

      pL->EnableLayer( false );
