...
/// \file
/// \brief  implementation of the FrameProfiler class which should
...
/// \param pszDLLNameWithoutExtension the name of the GPA dll to load without
///      the ".dll" extension
/// \return true if either the external or internal dll could be loaded; false otherwise
//-----------------------------------------------------------------------------
bool FrameProfiler::LoadProfilerDLL(GPA_SUPPORTED_API api)
{
...
void FrameProfiler::DoCounterSelect(TextCommandResponse& rSelectionRequest)
{
    ...
    {
        if (openStatus == GPA_STATUS_ERROR_CATALYST_VER_UNSUPPORTED_SI)
        {
            rSelectionRequest.SendError("Catalyst version 12.9 is unsupported for OpenGL profiles on 7xxx series cards. Please switch to a different version and try again.");
            return;
        }
...
void FrameProfiler::SendCounterInformationString(CommandResponse& rRequest)
{
    ...
    if (openStatus != GPA_STATUS_OK)
    {
        if (openStatus == GPA_STATUS_ERROR_CATALYST_VER_UNSUPPORTED_SI)
        {
            rRequest.SendError("Catalyst version 12.9 is unsupported for OpenGL profiles on 7xxx series cards. Please switch to a different version and try again.");
            return;
        }
...
bool FrameProfiler::HandleProfilerRequest(void* deviceToMonitor, CommandResponse& rProfilerXML, const char* configSectionLabel)
{
    if (m_GPALoader.Loaded())
    {
        ...
        if (openStatus != GPA_STATUS_OK)
        {
            if (openStatus == GPA_STATUS_ERROR_CATALYST_VER_UNSUPPORTED_SI)
            {
                Log(logERROR, "Catalyst version 12.9 is unsupported for OpenGL profiles on 7xxx series cards. Please switch to a different version and try again.");
                return false;
            }
...
void FrameProfiler::BeginFrame()
{
    ...
    if (needToOpenCounters)
    {
        ...
        if (openStatus != GPA_STATUS_OK)
        {
            if (openStatus == GPA_STATUS_ERROR_CATALYST_VER_UNSUPPORTED_SI)
            {
                std::string errorMsg = "Catalyst version 12.9 is unsupported for OpenGL profiles on 7xxx series cards.";
...
std::string FrameProfiler::GetStatusString(GPA_Status status)
{
    ...
    switch (status)
    {
        ...
        case GPA_STATUS_ERROR_CATALYST_VER_UNSUPPORTED_SI :
            result = "Catalyst Version unsupported on 7xxx series cards";
            break;
...
GPA_Status FrameProfiler::StatusCheck(GPA_Status status)
{
    switch (status)
    {
        ...
        case GPA_STATUS_ERROR_CATALYST_VER_UNSUPPORTED_SI :
            assert(!"Catalyst Version unsupported on 7xxx series cards");
            break;
