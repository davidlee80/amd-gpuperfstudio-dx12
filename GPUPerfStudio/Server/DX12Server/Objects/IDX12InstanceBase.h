...
public:
    ...
    /// \param inRealInterface The real interface pointer that the runtime gives us.
    ...
    //--------------------------------------------------------------------------
    IDX12InstanceBase(IUnknown* inRuntimeInstance, IUnknown* inWrapperInterface, eObjectType inObjectType, GPS_DX12CreateInfoBase* inCreateInfo);
    ...
    /// \returns The GPS_ID3D12Device instance that created the wrapped DeviceChild.
    //--------------------------------------------------------------------------
    virtual void* GetParentDeviceHandle() const { return mParentDevice; }

    //--------------------------------------------------------------------------
    /// Set the parent device for this wrapper object.
    /// \param inParentDevice A GPS_ID3D12Device wrapper instance that created this object.
    //--------------------------------------------------------------------------
    void SetParentDeviceHandle(GPS_ID3D12Device* inParentDevice) { mParentDevice = inParentDevice; }

    //--------------------------------------------------------------------------
    /// Retrieve a pointer to the handle that the application is using.
    /// \returns A void* to the real API object instance. Always a GPS_ID3D12[Something] instance.
    //--------------------------------------------------------------------------
    ...
    /// @DX12TODO: Must implement for wrapped DX12 objects. Will/should this use ID3D12Object::GetPrivateData?
    ...
    /// \returns A polymorphic GPS_DX12CreateInfoBase instance with the CreateInfo for this instance.
    //--------------------------------------------------------------------------
    GPS_DX12CreateInfoBase* GetCreateInfoStruct() const { return mCreateInfo; }
    ...
    //--------------------------------------------------------------------------
    /// The wrapper instance (GPS_ID3D...) used to hook interface calls.
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    GPS_DX12CreateInfoBase* mCreateInfo;

    //--------------------------------------------------------------------------
    /// A pointer to the parent device that created this instance.
    //--------------------------------------------------------------------------
    GPS_ID3D12Device* mParentDevice;
};
