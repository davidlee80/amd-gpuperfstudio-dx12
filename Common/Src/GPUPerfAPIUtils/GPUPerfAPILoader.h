...
public:
    ...
    /// \param[in] dllPath path to load GPA Dlls from
    /// \param[in] api the API to load GPA for
    /// \param[out] errorMessage the error message if loading fails
    /// \return true if the GPA Dll is successfully loaded, false otherwise
    bool Load(const char* dllPath, GPA_API_Type api, const char** errorMessage);
